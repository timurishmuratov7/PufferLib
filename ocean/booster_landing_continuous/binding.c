...
void my_log(Log* log, Dict* out) {
    ...
    dict_set(out, "mean_main_throttle", log->main_action_rate);
    dict_set(out, "mean_positive_attitude_command", log->left_action_rate);
    dict_set(out, "mean_negative_attitude_command", log->right_action_rate);
    dict_set(out, "mean_absolute_attitude_command",
        log->left_action_rate + log->right_action_rate);
    dict_set(out, "main_throttle_near_off_rate",
        log->main_throttle_near_off_rate);
    dict_set(out, "main_throttle_partial_rate",
        log->main_throttle_partial_rate);
    dict_set(out, "main_throttle_near_full_rate",
        log->main_throttle_near_full_rate);
    dict_set(out, "attitude_command_near_limit_rate",
        log->attitude_command_near_limit_rate);
    dict_set(out, "mean_raw_main_action", log->mean_raw_main_action);
    dict_set(out, "mean_absolute_raw_main_action",
        log->mean_absolute_raw_main_action);
    dict_set(out, "mean_absolute_raw_attitude_action",
        log->mean_absolute_raw_attitude_action);
    dict_set(out, "mean_main_throttle_altitude_0_100",
        log->action_steps_altitude_0_100 > 0.0f
            ? log->main_throttle_altitude_0_100_sum
                / log->action_steps_altitude_0_100
            : 0.0f);
    dict_set(out, "mean_absolute_attitude_command_altitude_0_100",
        log->action_steps_altitude_0_100 > 0.0f
            ? log->absolute_attitude_altitude_0_100_sum
                / log->action_steps_altitude_0_100
            : 0.0f);
    dict_set(out, "mean_main_throttle_altitude_100_500",
        log->action_steps_altitude_100_500 > 0.0f
            ? log->main_throttle_altitude_100_500_sum
                / log->action_steps_altitude_100_500
            : 0.0f);
    dict_set(out, "mean_absolute_attitude_command_altitude_100_500",
        log->action_steps_altitude_100_500 > 0.0f
            ? log->absolute_attitude_altitude_100_500_sum
                / log->action_steps_altitude_100_500
            : 0.0f);
    dict_set(out, "mean_main_throttle_altitude_500_plus",
        log->action_steps_altitude_500_plus > 0.0f
            ? log->main_throttle_altitude_500_plus_sum
                / log->action_steps_altitude_500_plus
            : 0.0f);
    dict_set(out, "mean_absolute_attitude_command_altitude_500_plus",
        log->action_steps_altitude_500_plus > 0.0f
            ? log->absolute_attitude_altitude_500_plus_sum
                / log->action_steps_altitude_500_plus
            : 0.0f);
