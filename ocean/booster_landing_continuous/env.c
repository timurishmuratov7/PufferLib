...
static float main_throttle_from_action(float action) {
    return 0.5f * (tanhf(action) + 1.0f);
}

static float attitude_command_from_action(float action) {
    return tanhf(action);
}
...
static float stopping_distance_risk(BoosterLanding* env) {
    float altitude = fmaxf(env->altitude, 0.0f);
    float downward_speed = fmaxf(-env->velocity, 0.0f);
    ...
    return clampf(distance_risk, 0.0f, 1.0f);
}

static float required_fuel_risk(BoosterLanding* env) {
    float vertical_thrust = env->thrust * fmaxf(cosf(env->angle), 0.0f);
    ...
    float fuel_risk = required_fuel > 0.0f
        ...
        : 0.0f;
    return clampf(fuel_risk, 0.0f, 1.0f);
}

static float landing_safety_risk(BoosterLanding* env) {
    return fmaxf(stopping_distance_risk(env), required_fuel_risk(env));
}
...
void c_init(BoosterLanding* env) {
    ...
    env->reset_angular_velocity_min = -0.05f;
    env->reset_angular_velocity_max = 0.05f;
    env->dry_mass = 120.0f;
...
void c_reset(BoosterLanding* env) {
    ...
    env->episode_main_throttle_near_off_steps = 0.0f;
    env->episode_main_throttle_partial_steps = 0.0f;
    env->episode_main_throttle_near_full_steps = 0.0f;
    env->episode_attitude_command_near_limit_steps = 0.0f;
    env->episode_raw_main_action_sum = 0.0f;
    env->episode_absolute_raw_main_action_sum = 0.0f;
    env->episode_absolute_raw_attitude_action_sum = 0.0f;
    env->episode_main_throttle_altitude_0_100_sum = 0.0f;
    env->episode_absolute_attitude_altitude_0_100_sum = 0.0f;
    env->episode_action_steps_altitude_0_100 = 0.0f;
    env->episode_main_throttle_altitude_100_500_sum = 0.0f;
    env->episode_absolute_attitude_altitude_100_500_sum = 0.0f;
    env->episode_action_steps_altitude_100_500 = 0.0f;
    env->episode_main_throttle_altitude_500_plus_sum = 0.0f;
    env->episode_absolute_attitude_altitude_500_plus_sum = 0.0f;
    env->episode_action_steps_altitude_500_plus = 0.0f;
...
static void add_log(BoosterLanding* env, bool success, bool timeout,
    ...
    env->log.main_throttle_near_off_rate +=
        env->episode_main_throttle_near_off_steps / episode_steps;
    env->log.main_throttle_partial_rate +=
        env->episode_main_throttle_partial_steps / episode_steps;
    env->log.main_throttle_near_full_rate +=
        env->episode_main_throttle_near_full_steps / episode_steps;
    env->log.attitude_command_near_limit_rate +=
        env->episode_attitude_command_near_limit_steps / episode_steps;
    env->log.mean_raw_main_action +=
        env->episode_raw_main_action_sum / episode_steps;
    env->log.mean_absolute_raw_main_action +=
        env->episode_absolute_raw_main_action_sum / episode_steps;
    env->log.mean_absolute_raw_attitude_action +=
        env->episode_absolute_raw_attitude_action_sum / episode_steps;
    env->log.main_throttle_altitude_0_100_sum +=
        env->episode_main_throttle_altitude_0_100_sum;
    env->log.absolute_attitude_altitude_0_100_sum +=
        env->episode_absolute_attitude_altitude_0_100_sum;
    env->log.action_steps_altitude_0_100 +=
        env->episode_action_steps_altitude_0_100;
    env->log.main_throttle_altitude_100_500_sum +=
        env->episode_main_throttle_altitude_100_500_sum;
    env->log.absolute_attitude_altitude_100_500_sum +=
        env->episode_absolute_attitude_altitude_100_500_sum;
    env->log.action_steps_altitude_100_500 +=
        env->episode_action_steps_altitude_100_500;
    env->log.main_throttle_altitude_500_plus_sum +=
        env->episode_main_throttle_altitude_500_plus_sum;
    env->log.absolute_attitude_altitude_500_plus_sum +=
        env->episode_absolute_attitude_altitude_500_plus_sum;
    env->log.action_steps_altitude_500_plus +=
        env->episode_action_steps_altitude_500_plus;
...
void c_step(BoosterLanding* env) {
    ...
    float raw_main_action = env->actions[0];
    float raw_attitude_action = env->actions[1];
    float main_throttle = main_throttle_from_action(raw_main_action);
    float policy_attitude_command = attitude_command_from_action(
        raw_attitude_action);
    float attitude_command = env->horizontal_reflected
        ? -policy_attitude_command : policy_attitude_command;
    float positive_attitude_command = fmaxf(attitude_command, 0.0f);
    float negative_attitude_command = fmaxf(-attitude_command, 0.0f);
    env->episode_main_action_steps += main_throttle;
    env->episode_left_action_steps += positive_attitude_command;
    env->episode_right_action_steps += negative_attitude_command;
    env->episode_main_throttle_near_off_steps +=
        main_throttle <= 0.05f ? 1.0f : 0.0f;
    env->episode_main_throttle_partial_steps +=
        main_throttle > 0.05f && main_throttle < 0.95f ? 1.0f : 0.0f;
    env->episode_main_throttle_near_full_steps +=
        main_throttle >= 0.95f ? 1.0f : 0.0f;
    env->episode_attitude_command_near_limit_steps +=
        fabsf(policy_attitude_command) >= 0.95f ? 1.0f : 0.0f;
    env->episode_raw_main_action_sum += raw_main_action;
    env->episode_absolute_raw_main_action_sum += fabsf(raw_main_action);
    env->episode_absolute_raw_attitude_action_sum +=
        fabsf(raw_attitude_action);

    if (env->altitude < 100.0f) {
        env->episode_main_throttle_altitude_0_100_sum += main_throttle;
        env->episode_absolute_attitude_altitude_0_100_sum +=
            fabsf(policy_attitude_command);
        env->episode_action_steps_altitude_0_100 += 1.0f;
    } else if (env->altitude < 500.0f) {
        env->episode_main_throttle_altitude_100_500_sum += main_throttle;
        env->episode_absolute_attitude_altitude_100_500_sum +=
            fabsf(policy_attitude_command);
        env->episode_action_steps_altitude_100_500 += 1.0f;
    } else {
        env->episode_main_throttle_altitude_500_plus_sum += main_throttle;
        env->episode_absolute_attitude_altitude_500_plus_sum +=
            fabsf(policy_attitude_command);
        env->episode_action_steps_altitude_500_plus += 1.0f;
    }

    float requested_main_fuel = env->fuel_burn_rate * env->dt * main_throttle;
    float requested_side_fuel = env->side_fuel_burn_rate * env->dt
        * fabsf(attitude_command);
    float requested_fuel = requested_main_fuel + requested_side_fuel;
    float fuel_scale = requested_fuel > 0.0f
        ? fminf(env->fuel / requested_fuel, 1.0f) : 0.0f;
    float effective_main_throttle = main_throttle * fuel_scale;
    float effective_attitude_command = attitude_command * fuel_scale;
    float mass = current_mass(env);
    float main_accel = env->thrust * effective_main_throttle / mass;
    float side_accel = env->side_thrust * effective_attitude_command / mass;
    float angular_accel = env->side_thrust * env->moment_arm
        * effective_attitude_command / env->moment_inertia;
    float main_fuel_used = requested_main_fuel * fuel_scale;
    float side_fuel_used = requested_side_fuel * fuel_scale;
    env->fuel -= main_fuel_used + side_fuel_used;
    env->fuel = fmaxf(env->fuel, 0.0f);
    env->episode_main_fuel_used += main_fuel_used;
    env->episode_side_fuel_used += side_fuel_used;
    ...
    env->angular_velocity *= ANGULAR_DAMPING;
...
Client* make_client(BoosterLanding* env) {
    ...
    InitWindow(env->width * RENDER_SCALE, env->height * RENDER_SCALE,
        "PufferLib Continuous Booster Landing");
    SetTargetFPS(60);
...
void c_render(BoosterLanding* env) {
    ...
    float main_throttle = main_throttle_from_action(env->actions[0]);
    float policy_attitude_command = attitude_command_from_action(env->actions[1]);
    float attitude_command = env->horizontal_reflected
        ? -policy_attitude_command : policy_attitude_command;
    bool burning = main_throttle > 0.001f && env->fuel > 0.0f;
    float mass = current_mass(env);
    ...
    DrawText(TextFormat("Throttle       %6.1f%%", 100.0f * main_throttle),
        18, 242, 16, burning ? warn : muted);

    DrawRectangle(8, env->height - 54, 286, 42, panel);
    DrawText(TextFormat("Step %d / %d", env->tick, env->max_episode_steps), 18, env->height - 46, 16, RAYWHITE);
    DrawText(TextFormat("SPACE throttle | A/D attitude (%+.2f)", attitude_command),
        18, env->height - 26, 16, muted);
